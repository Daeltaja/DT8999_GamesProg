...
public class FoodTemp : MonoBehaviour {
	...
	void Update () 
	{
		...
		{
			GetComponent<Renderer>().material.color = Color.red; //change the color, which belongs in the renderer, to red
			Debug.Log("Too Hot");
		}
		//else if its not greater than too hot, if currentTemp is less than tooCold
		else if(currentTemp < tooCold)
		{
			GetComponent<Renderer>().material.color = Color.blue; //change the color, which belongs in the renderer, to blue
			Debug.Log ("Too Cold");
		}
		else //otherwise if it's neither do this
		{
			GetComponent<Renderer>().material.color = Color.green; 	//change the color, which belongs in the renderer, to blue
